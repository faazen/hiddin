...
class ChatRoom extends Component {
  ...
  componentDidMount() {
    ...
    const Other_mobilenumber = route?.params?.contactData.number;
    this.setState({ Other_Mo_id: Other_mobilenumber });
    const contactName = route?.params?.contactData.name;
    const ProfileName = contactName;
    ...
    services
      ...
      .then((response) => {
        ...
        if (response.message === "room created") {
          this.setState({ Room_id: response.response[0].room_id });
          this.socket.emit("joinRoom", this.state.Room_id);
          this.GetMsg();
        } else {
          ToastMsg(response.message, "danger");
        }
  ...
  GetMsg() {
    let apiName = "getmsg" + "/" + this.state.Room_id;

    services
      .get(apiName)
      .then((response) => {
        console.log(" getmsg  chatMessages----> ", response);
        if (response.status === "Success") {
          this.setState({ chatMessages: response.result });
        } else {
          ToastMsg(response.message, "danger");
        }
      })
  ...
  renderItemIS = (item, index) => {
    console.log("item", this.state.Other_Mo_id);
              ...
              <View style={styles.RightChat}>
                ...
                >
                  ...
                  <Modal
                    ...
                    isOpen={this.state.isImageViewVisible}
                    onClosed={() =>
                  ...
                  >
                    <ImageView
                      images={{
                        uri: "http://51.15.204.121/hiddenly/Hiddenly/Media/status/status_16341285208802276.jpg",
                      }}
                      imageIndex={0}
                      isVisible={true}
                    />
